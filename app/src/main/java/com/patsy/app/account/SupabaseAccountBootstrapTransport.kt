...
class SupabaseAccountBootstrapTransport(
    ...
    private fun parse(body: String): RemoteAccountBootstrapResult = try {
        ...
        val capabilities = root.getJSONArray("capabilities").let { values ->
            buildSet {
                repeat(values.length()) { add(enum("capability", values.getString(it), OwnerCapability.entries.toTypedArray())) }
            }
        }
        val restrictions = root.getJSONArray("restrictions").let { values ->
            buildSet {
                repeat(values.length()) { add(enum("restriction", values.getString(it), ShellRestriction.entries.toTypedArray())) }
            }
        }
        RemoteAccountBootstrapResult.Available(
            AccountBootstrap(
                canonicalUserId = root.getString("canonical_user_id"),
                profileReadiness = enum("profile_readiness", root.getString("profile_readiness"), ProfileReadiness.entries.toTypedArray()),
                onboardingState = enum("onboarding_state", root.getString("onboarding_state"), OnboardingState.entries.toTypedArray()),
                ageState = enum("age_state", root.getString("age_state"), TrustedAgeState.entries.toTypedArray()),
                capabilities = capabilities,
                accountStatus = enum("account_status", root.getString("account_status"), AccountStatus.entries.toTypedArray()),
                settingsAvailability = enum("settings_availability", root.getString("settings_availability"), SettingsAvailability.entries.toTypedArray()),
