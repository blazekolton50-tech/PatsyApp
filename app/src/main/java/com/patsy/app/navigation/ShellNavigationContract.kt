...
sealed interface NavigationDecision {
    ...
    data class Denied(val requestedRouteId: String, val fallback: ShellDestination, val reason: NavigationDenial) : NavigationDecision
}

enum class NavigationDenial { UNKNOWN_ROUTE, ONBOARDING_INCOMPLETE, ACCOUNT_UNAVAILABLE, RESTRICTED, CAPABILITY_MISSING }
...
object ShellNavigationGate {
    ...
    fun resolve(routeId: String, account: AccountBootstrap, localAgeChoice: String? = null): NavigationDecision {
        val route = ShellNavigationContract.route(routeId)
            ?: return NavigationDecision.Denied(routeId, ShellDestination.SAFE_PROTECTED, NavigationDenial.UNKNOWN_ROUTE)
        ...
        if (account.accountStatus != AccountStatus.ACTIVE) {
            return NavigationDecision.Denied(routeId, ShellDestination.SAFE_PROTECTED, NavigationDenial.ACCOUNT_UNAVAILABLE)
        }
        ...
        if (account.isProtectedMode && route.blockedBy in setOf(ShellRestriction.DMS_RESTRICTED, ShellRestriction.PROVIDERS_RESTRICTED)) {
            return NavigationDecision.Denied(routeId, route.fallback, NavigationDenial.RESTRICTED)
        }
