...
class SupabaseHttpAuthTransport(
    ...
    override suspend fun login(identifier: LoginIdentifier, password: CharArray): RemoteAuthResult =
        withContext(Dispatchers.IO) {
            requestAuthLogin(identifier, password)
        }
    ...
    private fun requestAuthLogin(identifier: LoginIdentifier, password: CharArray): RemoteAuthResult {
        return try {
    ...
    private fun requestRefresh(refreshToken: String): RemoteAuthResult {
        return try {
    ...
    private fun parseSupabaseSession(root: JSONObject): RemoteAuthResult {
        ...
        if (access.isBlank() || refresh.isBlank() || userId.isBlank()) {
            return RemoteAuthResult.Failure(RemoteAuthFailure.Unknown)
        }
        val expiresAt = root.optLong("expires_at", 0L).takeIf { it > 0L }?.times(1000L)
            ?: (System.currentTimeMillis() + root.optLong("expires_in", 3600L) * 1000L)
    ...
    private fun fetchOwnUsername(userId: String, accessToken: String): String? {
        return try {
